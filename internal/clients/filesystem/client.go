// Package filesystem provides an HTTP client wrapper for the filesystem-gateway API.
package filesystem
...
// Client wraps the filesystem-gateway HTTP API.
type Client struct {
	base *clients.Base
}

// NewClient creates a new filesystem-gateway client.
...
// writeFileRequest is the JSON body for POST /servers/{name}/files/write.
...
func (c *Client) WriteFile(ctx context.Context, name, path, content string, uid, gid int) error {
	...
	if err := c.base.DoNoContent(ctx, "POST", "/servers/"+name+"/files/write", body); err != nil {
		return fmt.Errorf("write file for %q: %w", name, err)
	}
	return nil
}

// migrateServerRequest is the JSON body for POST /servers/{name}/migrate.
...
func (c *Client) RenameServer(ctx context.Context, oldName, newName string) error {
	body := migrateServerRequest{NewName: newName}
	if err := c.base.DoNoContent(ctx, "POST", "/servers/"+oldName+"/migrate", body); err != nil {
