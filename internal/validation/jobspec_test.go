...
var testVolumePrefixes = []string{"/mnt/data/"}
...
func TestValidateJobSpec_Valid(t *testing.T) {
	err := ValidateJobSpec(validHCL, nil, testVolumePrefixes, "", "")
	if err != nil {
...
`
	err := ValidateJobSpec(hcl, nil, testVolumePrefixes, "", "")
	if err == nil {
...
func TestValidateJobSpec_MultipleVolumePrefixes(t *testing.T) {
	hcl := `
job "gitea" {
  group "server" {
    task "app" {
      driver = "docker"
      config {
        volumes = ["/mnt/data/gitea/data:/data"]
      }
      resources { cpu = 1000; memory = 1024 }
    }
  }
...
`
	// Fails with only minecraft prefix.
	err := ValidateJobSpec(hcl, nil, []string{"/mnt/data/minecraft/"}, "", "")
	if err == nil {
		t.Fatal("expected error for volume outside minecraft prefix")
	}

	// Passes with both prefixes.
	err = ValidateJobSpec(hcl, nil, []string{"/mnt/data/minecraft/", "/mnt/data/gitea/"}, "", "")
	if err != nil {
		t.Fatalf("unexpected error with multiple prefixes: %v", err)
	}
}
...
`
	err := ValidateJobSpec(hcl, []string{"custom.example.com/"}, testVolumePrefixes, "", "")
	if err != nil {
